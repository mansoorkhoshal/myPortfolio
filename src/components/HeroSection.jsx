...
export const HeroSection = () => {
  ...
  const achievements = [
    { number: "1+", label: "Years in Production", icon: <Shield className="h-8 w-8" /> },
    { number: "10+", label: "Projects Delivered", icon: <TrendingUp className="h-8 w-8" /> },
    { number: "100%", label: "Client Satisfaction", icon: <Award className="h-8 w-8" /> },
    { number: "35+", label: "Projects completed", icon: <Zap className="h-8 w-8" /> }
  ];
  ...
  const handleViewResume = () => {
    ...
    window.open('/public/mansoorahmad_dev_CV.pdf', '_blank', 'noopener,noreferrer');
  };

  return (
    <section id="hero" className="relative min-h-screen flex items-center justify-center px-4 pt-2 pb-20 sm:px-6 lg:px-8 overflow-hidden bg-gradient-to-br from-background via-background/95 to-primary/10" ref={ref}>
      ...
      <div className="container max-w-7xl mx-auto w-full mt-16 sm:mt-0">
        <motion.div className="flex flex-col lg:flex-row items-center justify-between gap-16 lg:gap-20" initial="hidden" animate={isInView ? "visible" : "hidden"} variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.25, delayChildren: 0.5 } } }}>
          ...
          <div className="flex-1 text-center lg:text-left max-w-2xl mx-auto lg:mx-0">
            <motion.div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 text-primary text-sm font-medium mt-3 backdrop-blur-sm" variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}>
            ...
            <motion.h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-tight tracking-tight" variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}>
              <span className="block sm:inline text-foreground whitespace-normal sm:whitespace-nowrap">
                I'm Mansoor Ahmad
              </span>
              ...
              <motion.div className="bg-background/90 border border-border rounded-2xl p-4 backdrop-blur-sm shadow-2xl w-full group hover:shadow-3xl transition-all duration-500" whileHover={{ y: -4 }} transition={{ type: "spring", stiffness: 400, damping: 25 }}>
                ...
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div className="flex-1 text-center">
                    <div className="text-sm font-mono font-semibold text-muted-foreground">mansoor_Intro.js</div>
                  </div>
                </div>
                ...
                <motion.div className="absolute -bottom-3 left-1/2 transform -translate-x-1/2 bg-background/90 backdrop-blur-sm px-8 py-2 rounded-xl border border-border shadow-lg text-center" initial={{ scale: 0, y: 20 }} animate={{ scale: 1, y: 0 }} transition={{ delay: 2, type: "spring" }}>
